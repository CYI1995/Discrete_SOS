...
def load_exact_h(h):
    """
    Load beta and exact parent-Hamiltonian absolute gap.
    ...
    # Allow an old file with shape (n, 1).
    if gap.ndim == 2 and gap.shape[1] == 1:
        gap = gap[:, 0]

    if beta.ndim != 1:
        raise ValueError(
            f"U={h:g}: exact beta must be one-dimensional; "
            f"received shape {beta.shape}."
        )

    if gap.shape != (beta.size,):
        raise ValueError(
            f"U={h:g}: exact absolute_gap has shape {gap.shape}, "
            f"but the expected shape is {(beta.size,)}."
        )
    ...
    if beta.size > 1 and not np.all(
        ...
        raise ValueError(
            f"U={h:g}: exact beta values must be strictly increasing."
        )

    if not np.all(np.isfinite(beta)):
        raise ValueError(
            f"U={h:g}: exact beta contains NaN or inf."
        )

    if not np.all(np.isfinite(gap)):
        raise ValueError(
            f"U={h:g}: exact absolute_gap contains NaN or inf."
        )

    if np.any(gap <= 0.0):
        raise ValueError(
            f"U={h:g}: exact absolute_gap contains nonpositive values."
        )
...
for h, krylov_data, exact_data in zip(
    h_values,
    krylov_datasets,
    exact_datasets,
):
    krylov_beta = krylov_data[0]
    exact_beta = exact_data[0]
    ...
    if exact_beta.size == 0:
        exact_status = "no valid beta"
    elif exact_beta.size < N_BETA:
        exact_status = (
            f"{exact_beta.size}/{N_BETA}, "
            f"last beta={exact_beta[-1]:.4f}"
        )
    else:
        exact_status = (
            f"{exact_beta.size}/{N_BETA}, complete"
        )

    print(
        f"U={h:g}: "
        f"Krylov [{krylov_status}], "
        f"exact [{exact_status}]"
    )
