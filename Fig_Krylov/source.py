...
def bin_to_dec(bin,size):
    
    num = 0
    for i in range(size):
        num = num + 2**(size-1-i)*bin[i]
...
def YY_pair(site1,site2,L):

    XX = XX_pair(site1,site2,L)
    ZZ = ZZ_pair(site1,site2,L)
...
def trace_onenorm(M):
    ...
    norm = 0
    for i in range(dim):
        norm = norm + 0.5 * math.sqrt(abs(eig[i]))
