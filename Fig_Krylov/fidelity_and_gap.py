...
REL_GAP_TOL = 1e-8      # cutoff for the unshifted absolute parent-Hamiltonian gap
...
def checked_ground_state(parent, shift):
    ...
    The diagonalized matrix is M_shifted = exp(-2 * shift) M_unshifted.  The
    relative gap

        relative_gap = shifted_gap / ||M_shifted||_inf

    is invariant under that rescaling -- the factor cancels between numerator
    and denominator -- so it is compared with REL_GAP_TOL directly, with no
    log-domain treatment needed.

    The absolute gap, absolute_gap = exp(2 * shift) * shifted_gap, is still
    computed (in log space, to avoid overflow) and stored, but only as a
    diagnostic.  Likewise for the Davis--Kahan quantity sin_theta.  Apart from
    a nonpositive/nonfinite eigensolver gap or scale, truncation is controlled
    only by REL_GAP_TOL.
    """
    ...
    # Diagnostic only.  log(gap_unshifted) = log(gap_shifted) + 2 * shift;
    # if that overflows, +inf is an honest value (it is certainly large).
    ...
    if relative_gap <= REL_GAP_TOL:          # <-- 唯一的截断判据
        return None
...
def save_results(results, path=RESULTS_PATH):
    ...
    payload["metadata"] = {
        ...
        "angle_tol": ANGLE_TOL,
        "truncation_criterion": "relative_gap <= rel_gap_tol",
        "relative_gap_convention": (
            "shifted_gap / ||M_shifted||_inf; invariant under the global "
            "factor exp(-2*x*W), hence equal to the unshifted relative gap"
        ),
        "absolute_gap_convention": (
            "gap of the unshifted parent Hamiltonian; "
            "gap_unshifted = exp(2*x*W) * gap_shifted (diagnostic only)"
        ),
    }
